...
| 4 | roads (resolve_road, draw_road, pool) | resolver (seat, 7 tests) + `AStacktownRoad`, RoadFrame, reconcile, G mode (coordinator) | DONE 2026-09-06: drawn road and a lot on its frame proven live in a C++-owned game |
...
items: the board factory with the pinned spans (starter lot), ParcelId, the
design lane's road look. Night (L) landed in C++ on 2026-09-06 evening.
...
PIPELINE PROVEN EARLY (2026-09-06): Tools/package.sh produces a
self-contained Saved/Packaged/Mac/StacktownAlpha.app (universal, 1.3 GB,
about 90 s after the first cook) that launches and runs the C++ runtime. The packaged app runs the C++ camera and HUD only until steps 3, 4
